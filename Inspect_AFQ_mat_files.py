...
def Inspect_AFQ_mat_files(subj, working_dir, usehighres):
    ...
    if usehighres:
        dti_img_file = f'bin/sub-{subj}-visit2-MEMP-orig_magnet_space_bet.nii.gz'
    else:
        dti_img_file = f'bin/mpfcoreg12.nii.gz'

    # Load data
    mat_data = scipy.io.loadmat(op.join(mat_dir, mat_file))
    dti_space_img = nib.load(op.join(mat_dir, dti_img_file))

    # Extract image affine
    affine = dti_space_img.affine

    fg = mat_data['fg']
    fibers = fg['fibers']
    names = fg['name'] # tract region names

    for i in range(fibers.shape[1]):  # Loop over regions
