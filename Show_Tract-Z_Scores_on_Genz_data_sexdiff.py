...
z_score_filename = f"{metric}_node_sig_stats_from_ttest_sexdiff_notcorrected.csv"
...
for tid in tract_ids:
    ...
    # Load zscore values
    pvect = load_z_p_data(z_score_filepath, z_score_filename, tid.replace(' ', '_'))
    ...
    for sl in aligned_streamlines:
        ...
        # Make them gray
        colors = np.tile([0.4, 0.4, 0.4], (len(interpolated_p_values), 1))
        ...
        if check_orientation_flag == 1:
            colors = check_orientation(interpolated_p_values)
    ...
    window.record(
        scene=scene,
        out_path=op.join(out_folder, img_dir, f'{metric}_sexdiff_{tid_no_spaces}.png'),
        size=(1200, 1200))
